...
class _TelaTodoListState extends State<TelaTodoList>{
  ...
  void _carregarPreferencias() async {
    SharedPreferences _prefs = await SharedPreferences.getInstance();  // estabelece conexão com a memória
    _nome = _prefs.getString("nome") ?? "";  // pega as informações do nome
    _darkMode = _prefs.getBool("dark") ?? false;  // pega as informações do darkMode
    _chaveLista = "lista${_nome}";  // constrói a chave da lista
